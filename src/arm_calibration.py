...
with open(directory + "/TranslationMatrix.csv", "w", newline="") as outfile:
    writer = csv.writer(outfile)
    for i in range(len(trans_O_B_update)):
        writer.writerow(trans_O_B_update[i])

    outfile.close()

with open(directory + "/RotationMatrix.csv", "w", newline="") as outfile:
    writer = csv.writer(outfile)
    for i in range(len(rot_mat)):
        writer.writerow(rot_mat[i])
